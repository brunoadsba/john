#!/usr/bin/env python3
"""
SFT Etapa 4: Completo (5000+ exemplos, 3 épocas)
"""
import sys
import os
import argparse
...
def train_with_validation(
    ...
    """Treina modelo"""
    config = TrainingConfig.get_sft_config_for_etapa(4)
    
    logger.info("=" * 60)
    logger.info("SFT ETAPA 4: COMPLETO")
    logger.info("=" * 60)
    
    # Verifica pré-requisito
    status = status_manager.get_phase_status("sft")
    if status.get("etapas", {}).get("3", {}).get("status") != "completed":
        logger.error("❌ Etapa 3 deve estar completa")
        sys.exit(1)
    
    status_manager.update_phase_status("sft", "in_progress", etapa="4")
    ...
    try:
        ...
        status_manager.update_phase_status("sft", "completed", etapa="4", checkpoint_path=str(model_path))
        logger.info("✅ ETAPA 4 CONCLUÍDA!")
        return model_path
    except Exception as e:
        logger.error(f"Erro: {e}")
        status_manager.update_phase_status("sft", "error", etapa="4", error=str(e))
        raise


async def main():
    """Função principal"""
    parser = argparse.ArgumentParser(description="SFT Etapa 4: Completo")
    parser.add_argument("--dataset", type=str)
    parser.add_argument("--output-dir", type=str, default="models/finetuned/jonh-ft-v1-etapa4")
    parser.add_argument("--skip-validation", action="store_true")
    
    args = parser.parse_args()
    
    if not args.skip_validation:
        config = TrainingConfig.get_limits_for_etapa(4)
    ...
    if args.dataset:
        ...
        dataset_path = await prepare_dataset_from_db(database, "data/training/sft_dataset_etapa4.json", limit=None)
        await database.close()
    
    if not args.skip_validation:
        if not validate_dataset(dataset_path, min_samples=5000, min_quality=0.7):
            sys.exit(1)
